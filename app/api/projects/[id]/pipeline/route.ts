...
export async function PATCH(req: Request, { params }: Params) {
  try {
    ...
    if (!access.canEdit)  return NextResponse.json({ error: 'Accès en lecture seule' }, { status: 403 })

    const { data: project } = await supabase
      .from('projects').select('id, task_states').eq('id', id).single()
    ...
    if (error) console.warn('[pipeline] Update failed:', error.message)
