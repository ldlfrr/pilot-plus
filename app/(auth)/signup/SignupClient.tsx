...
import {
  ...
  ArrowRight, CheckCircle, Eye, EyeOff, Zap, ShieldCheck,
  Brain, Target, Radio,
} from 'lucide-react'
...
function SignupForm() {
  ...
  const [invitationInfo, setInvitationInfo] = useState<{
    project_name?: string; team_name?: string; inviter_name?: string
  } | null>(null)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSuccess(true)
    setTimeout(() => {
      router.push(inviteToken ? `/invite/${inviteToken}` : '/accueil')
      router.refresh()
    }, 1800)
  }
  ...
  const pwMeta = [
    null,
    { bars: 'bg-red-500',     label: 'text-red-400',    text: 'Faible' },
    { bars: 'bg-amber-400',   label: 'text-amber-400',  text: 'Moyen'  },
    { bars: 'bg-emerald-500', label: 'text-emerald-400',text: 'Fort'   },
  ]
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5">
        ...
        <div>
          ...
          <div className="relative">
            <User size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/25 pointer-events-none z-10" />
            <input
              ...
              style={inputStyle('fullName', fullName)}
            />
          </div>
        </div>
                ...
                {[1, 2, 3].map(n => (
                  <div
                    ...
                    className={cn(
                      ...
                      n <= pwStrength ? (pwMeta[pwStrength]?.bars ?? '') : 'bg-white/8',
                    )}
                  />
                ))}
              </div>
              <span className={cn('text-[11px] font-semibold', pwMeta[pwStrength]?.label)}>
                {pwMeta[pwStrength]?.text}
              </span>
            </div>
          )}
        </div>
        ...
        <button
          ...
          style={{
            ...
            boxShadow: isValid && !loading ? '0 4px 20px rgba(59,130,246,0.30), inset 0 1px 0 rgba(255,255,255,0.15)' : 'none',
      ...
      <div className="flex items-center justify-center gap-2 mt-5 mb-5">
        ...
        <span className="text-[11px] text-white/22">Données sécurisées · Hébergées en Europe · RGPD</span>
      </div>

      {/* Divider */}
      <div className="flex items-center gap-3 mb-5">
        <div className="flex-1 h-px" style={{ background: 'rgba(255,255,255,0.06)' }} />
        <span className="text-[11px] text-white/22 font-medium">déjà un compte ?</span>
        <div className="flex-1 h-px" style={{ background: 'rgba(255,255,255,0.06)' }} />
      </div>
